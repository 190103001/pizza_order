...
public class PizzaOrder {
	...
	*/
	public static String orderPizza() { 
		...
		result += ":" + (cost + (toppingCost * numberOfToppings));
		return result;
   	}
	...
	*/
	public static int getTotalCost(String[] orders){
		...
		int[] cost = new int[orders.length];

		for(int i = 0; i < orders.length; i++) {
			if(orders[i] != null) {
				String[] strCost = orders[i].split(":");
				cost[i] = Integer.parseInt(strCost[1]);
			} else {
				continue; 
			}
		}
	...
	*/
	public static void confirmOrder(String[] orders, boolean discount){
		...
		// ADD LINES HERE
		if(discount) {
			double percent = DISCOUNT_AMOUNT / 100.0;
			int discountCost = (int) (cost - (cost * percent));
			System.out.println("-----------------------------");
			System.out.println("TOTAL with DISCOUNT (on presenting ID only!):");
			System.out.println(discountCost + "T");
		}
	...
	*/
	public static void printCurrentTime() {
		Date date = new Date();
		SimpleDateFormat formatForDate = new SimpleDateFormat("H:m");
		System.out.print(formatForDate.format(date));  
	}
